...
for (i in 1:length(baseline_effs)){
  ...
  df_efficacy_delta_raw.base_eff_grid <- expand_grid(d1 = d1s,#reduction in infection period/transmissibility
                                                     d2 = d2s,#reduction in mortality
                                                     e = 0.8,
                                                     tbsa=c(1,100,365),
                                                     model = "pars_le_covid") %>%
  ...
  df_efficacy_delta.base_eff_grid <- df_efficacy_delta_raw.base_eff_grid %>%
    ...
    select(red_bsa_infec, red_bsa_death, e, tbsa, i,d,d_rel,harm, econ_loss) %>%
