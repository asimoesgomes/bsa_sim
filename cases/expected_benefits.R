...
# Also note if other things need to be run prior 
# (seeing parameters in here not defined within the script)
...
exp_ben <- function(model, tbsa, R0, cfr=1, rm = FALSE) {
  # FUNCTION DOCUMENTATION FOLLOWING docstring PACKAGE.
  d1 <- 1.5 # DEFINE
  d2 <- 0.5 # DEFINE
  e <- 0.8 # DEFINE
  pars <- grab_2v_parms(model) # DESCRIBE
  ...
  pdeath <- default_pdeath # Where is this coming from?

  # DESCRIBE WHAT IS GOING ON AND PURPOSE
...
df_r0_cfr_raw.base_eff_grid <- expand_grid(tbsa = c(100,Ndays),
                                           ...
                                           #cfr=cfr_draw,
                                           model = "pars_le_covid") %>%
  mutate(data = pmap(list(model, tbsa, R0),
                     function(x, o, q) data.frame(value = exp_ben(x, o, q),
                     var = metric_nms))) %>% # DESCRIBE
  unnest(data) %>% # DESCRIBE
  spread(var, value) %>% # DESCRIBE
  #mutate(nab_factor = factor(round(e,3),levels=round(effs,3),labels=nab_factor_mod)) %>% 
  group_by(R0)  %>% #group_by(R0, cfr)  %>%
  mutate(d_rel = 1 - d / max(d)) %>% # DESCRIBE
  mutate(tbsa = factor(tbsa, levels = c(100, Ndays),
                       labels = c("100 day mission", "Default")))

# Sebastian: stopped providing comments here but you get the idea.

df_r0_cfr_raw.base_eff_grid <- df_r0_cfr_raw.base_eff_grid %>%
  mutate(econ_loss = exp(0.74 + 0.46 * log(d * 1000 / (Ndays / 365)))) %>% # PULL OUT MAGIC NUMBERS
  ...
  mutate(le_better = cut(r, seq(0, 1.0001, 0.2)
                         ...
                         #            "Full dose better")
  )) %>% # Please use more descriptive names than var
