odin_ode_2dose_bsa_v1 <- odin::odin({
  ...
  beta_matrix[,] <- contacts[i,j]*q[as.integer(t),j]*(Ibsa_pre[j]+Ibsa_post[j]*(1-(bsa_infec-1))+Inbsa[j])
  beta[] <- sum(beta_matrix[i,])
  
  va1[] <- 1#1/((1 + exp(ta[i] - t))*(1 + exp(50*(cumV1[i] - vstop[i]))))
  ...
  deriv(Ibsa_post[])   <-  gammaT[i]*Ibsa_pre[i] - 1/(1/gamma2[i]-1/gammaT[i])*Ibsa_post[i]
  deriv(Rnbsa[])       <-  (1-pdeath[i])*gamma2[i]*Inbsa[i] - phi[i]*Rnbsa[i] - va1[i]*v1*Rnbsa[i]*vrf/(S[i]+R[i])
  deriv(Rbsa[])       <-  (1-bsa_death*pdeath[i])*(1/(1/gamma2[i]-1/gammaT[i])*Ibsa_post[i]) - phi[i]*Rbsa[i] - va1[i]*v1*Rbsa[i]*vrf/(S[i]+R[i])
  deriv(R[])            <- (1-pdeath[i])*gamma2[i]*Inbsa[i] - phi[i]*Rnbsa[i] - va1[i]*v1*Rnbsa[i]*vrf/(S[i]+R[i])+
    (1-bsa_death*pdeath[i])*(1/(1/gamma2[i]-1/gammaT[i])*Ibsa_post[i]) - phi[i]*Rbsa[i] - va1[i]*v1*Rbsa[i]*vrf/(S[i]+R[i])
  deriv(Dnbsa[])       <-  pdeath[i]*(gamma2[i]*Inbsa[i])
  deriv(Dbsa[])       <-  bsa_death*pdeath[i]*(1/(1/gamma2[i]-1/gammaT[i])*Ibsa_post[i])
  deriv(D[])            <- pdeath[i]*(gamma2[i]*Inbsa[i])+bsa_death*pdeath[i]*(1/(1/gamma2[i]-1/gammaT[i])*Ibsa_post[i])
  ...
  deriv(I[]) <- gamma1[i]*E[i] - gamma2[i]*Inbsa[i] - 1/(1/gamma2[i]-1/gammaT[i])*Ibsa_post[i]
